...
def submit_review(request, product_id):
    url = request.META.get('HTTP_REFERER') # Get the curren url
    if request.method == 'POST':
        try:
            reviews = ReviewRating.objects.get(user__id=request.user.id, product__id=product_id) # __ use to access 
            form = ReviewForm(request.POST, instance=reviews) # instance is use TO check and update review
            form.save()
            ...
            if form.is_valid():
                ...
                data.rating = form.cleaned_data['rating'] # refering to product_detail.html name rating and review
